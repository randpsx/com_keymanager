 ...
 * Keymanager model.
 */
class KeymanagerModelBuilding extends JModelAdmin
{
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Building', $prefix = 'KeymanagerTable', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_keymanager.edit.building.data', array());
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__keymanager_buildings');
				$max = $db->loadResult();
