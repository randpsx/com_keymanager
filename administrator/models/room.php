 ...
 * Keymanager model.
 */
class KeymanagerModelRoom extends JModelAdmin
{
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Room', $prefix = 'KeymanagerTable', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_keymanager.edit.room.data', array());
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__keymanager_rooms');
				$max = $db->loadResult();
