...
class KeymanagerModelRequests extends JModelList
{
    ...
    $query->join('LEFT', '#__users AS uc ON uc.id=a.checked_out');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.requester_username LIKE '.$search.'  OR  a.access_card LIKE '.$search.' )');
            }
        }
    ...
    public function getItems()
    {
        ...
        foreach($items as $item){
					...
					$item->can_pickup = JText::_('COM_KEYMANAGER_REQUESTS_CAN_PICKUP_OPTION_' . strtoupper($item->can_pickup));
}
        return $items;
    }
}